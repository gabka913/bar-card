          ...
          `);continue}let s;if(s=o.attribute?r.attributes[o.attribute]:r.state,o.severity&&this._computeSeverityVisibility(s,t))continue;let l=n(this._hass,o.max),d=n(this._hass,o.min);l<=d&&(0===l&&0===d?(d=0,l=100):l=d+Math.max(1,.1*Math.abs(d))),o.limit_value&&(s=Math.min(s,l),s=Math.max(s,d)),isNaN(Number(s))||(0==o.decimal?s=Number(s).toFixed(0):o.decimal&&(s=Number(s).toFixed(o.decimal)));const h=Math.round(2*this._getLineHeightPx()),p=o.height??h;let g,m,f,v="stretch",b="0px 0px 0px 13px",$="right",_="row",y="left",w="height: 100%; width: 2px;";switch(o.direction){case"right":$="right",y="left";break;case"up":b="0px",$="top",_="column-reverse",y="bottom",w="height: 2px; width: 100%;"}switch(f=this._computeSeverityIcon(s,t)?this._computeSeverityIcon(s,t):o.icon?o.icon:r.attributes.icon?r.attributes.icon:u(c(o.entity),s),o.positions.icon){case"outside":g=nt`
            ...
            `;break;case"inside":T=nt`
              <bar-card-value
                ...
                >${o.complementary?l-s:s} ${S}</bar-card-value
              >
            `;break;case"off":b="0px"}let j="";s>this._stateArray[t]?(j="▲","up"==o.direction?this._animationState[t]="animation-increase-vertical":this._animationState[t]="animation-increase"):s<this._stateArray[t]?(j="▼","up"==o.direction?this._animationState[t]="animation-decrease-vertical":this._animationState[t]="animation-decrease"):this._animationState[t]=this._animationState[t],isNaN(Number(s))&&(j="");const N=this._computeBarColor(s,t);let P,U;const M=this._indicatorToggle[t]?"bar-card-indicator-fade-a":"bar-card-indicator-fade-b",H=j?`opacity:1; animation: ${M} 2s forwards;`:"";switch(o.positions.indicator){case"outside":P=nt`
              <bar-card-indicator
            ...
            `;break;case"inside":U=nt`
              <bar-card-indicator style="--bar-color: ${N}; ${H}">${j}</bar-card-indicator>
