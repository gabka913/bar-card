    ...
    `:it``}_createBarArray(){const t=[];for(let e=0;e<this._configArray.length;e++)(t.length+1)*this._config.columns==e&&t.push(this._config.columns),this._configArray.length==e+1&&t.push(this._configArray.length-t.length*this._config.columns);const e=[];for(let n=0;n<t.length;n++){const o=[];for(let e=0;e<t[n];e++){const t=n*this._config.columns+e,a=this._configArray[t],r=this._hass.states[a.entity];if(!r){o.push(it`
          ...
          `),"inside"===T&&(C=it`<bar-card-min>${d}${k}</bar-card-min>`),"inside"===j&&(O=it`<bar-card-max>${l}${k}</bar-card-max>`);let U=it``;if(!0===a.showPercentValue){const t=Math.round(c/l*1e4)/100;U=it`<bar-card-value-percent>${t}%</bar-card-value-percent>`}switch(a.positions.value){case"outside":P=it`
            ...
            `;break;case"inside":N=it`
              <bar-card-value
                class="${"inside"===T||"inside"===j?"":"up"==a.direction?"value-direction-up":"value-direction-right"}"
                >${a.complementary?l-c:c} ${k}${U}</bar-card-value
              >
            `;break;case"off":b="0px"}let M="";c>this._stateArray[t]?(M="▲","up"==a.direction?this._animationState[t]="animation-increase-vertical":this._animationState[t]="animation-increase"):c<this._stateArray[t]?(M="▼","up"==a.direction?this._animationState[t]="animation-decrease-vertical":this._animationState[t]="animation-decrease"):this._animationState[t]=this._animationState[t],isNaN(Number(c))&&(M="");const H=this._computeBarColor(c,t);let I,z;const R=this._indicatorToggle[t]?"bar-card-indicator-fade-a":"bar-card-indicator-fade-b",D=M?`opacity:1; animation: ${R} 2s forwards;`:"";switch(a.positions.indicator){case"outside":I=it`
              <bar-card-indicator
            ...
            `}const V=this._computePercent(c,t,l,d),L=this._computePercent(a.target,t,l,d);let B=V,F=this._computePercent(a.target,t,l,d);F<B&&(B=F,F=V);let W="";a.width&&(v="center",W=`width: ${a.width}`);const q=this._animationState[t];let J="right",K=100*V,X="animationbar-horizontal";"animation-increase-vertical"!=q&&"animation-decrease-vertical"!=q||(J="bottom",X="animationbar-vertical",K=100*(100-V)),o.push(it`
          <bar-card-card
            style="flex-direction: ${_}; align-items: ${v};"
          >
            ${g} ${I} ${A}
            <bar-card-background
              style="margin: ${b}; height: ${u}${"number"==typeof u?"px":""}; ${W}"
              data-config-index="${t}"
            ...
            >
              ...
              ${"on"===a.animation.state?it`
                    <bar-card-animationbar
                      style="animation: ${q} ${a.animation.speed}s infinite ease-out;
                             --bar-percent: ${K}%;
                             --bar-color: ${H};
