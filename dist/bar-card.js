...
export class BarCard extends LitElement {
  ...
  _handleAction(ev) {
    if (this._hass && ev.detail && ev.detail.action) {
      ...
      const configIndex = parseInt(ev.target.dataset.configIndex || '0');
      const config = this._configArray[configIndex] || this._config;
      
      handleAction(ev.target, this._hass, config, ev.detail.action);
    }
  }
  ...
  _createBarArray() {
    ...
    for (let i = 0; i < columnsArray.length; i++) {
      ...
      for (let x = 0; x < columnsArray[i]; x++) {
        ...
        const barHeight = config.height ?? defaultHeight;
  ...
  _computePercent(value, index, max, min) {
    const config = this._configArray[index];
    const numberValue = Number(value);
